...
def link_filter(links):
    ...
    for link in links:
        ...
        if (
            "indiana.edu" in link
            or "iu.edu" in link
            or "{{Form-Link}}" in link
            or "{{Form-Survey-Link}}" in link
        ):
            ...
            if (
                ...
                and "tel:" not in link
                and ".jpg" not in link
                and "https://one.iu.edu" not in link
                and "machform" not in link
                and "go.iu.edu" not in link
            ):
...
def content_grabber(html):
    ...
    for link in soup.findAll("a"):
        ...
        if (
            "indiana.edu" in link["href"]
            or "iu.edu" in link["href"]
            or "{{Form-Link}}" in link["href"]
            or "{{Form-Survey-Link}}" in link["href"]
        ):
            if (
                "mailto:" not in link["href"]
                and ".png" not in link["href"]
                and "tel:" not in link["href"]
                and ".jpg" not in link["href"]
                and "https://one.iu.edu" not in link["href"]
                and "machform" not in link["href"]
                and "go.iu.edu" not in link["href"]
            ):
                ...
                if len(link.contents) == 1:
                    if "#" in link["href"]:
                        ...
                        dirty_content = (
                            unneeded_space_remover(str(link.contents[0]))
                            .replace(" ", "-")
                            .lower()
                            .strip("'")
                        )
                ...
                # rinse and repeat
                    ...
                    if "#" in link["href"]:
                        ...
                        anchor = "#" + link["href"].split("#")[1]
                        dirty_content = (
                            unneeded_space_remover(str(link.contents[2]))
                            .replace(" ", "-")
                            .lower()
                            .strip("'")
                            + "-button"
...
def HTML_link_replacer(html, original_link_list, new_link_list):
    ...
    for link in original_link_list:
        html = html.replace(
            'href="' + original_link_list[i] + '"', 'href="' + new_link_list[i] + '"', 1
        )
        i += 1
...
# added this function since it seemed like a change in one of the libraries I'm using was adding in a ton of spaces to the link text before it went through the UTM process, resulting in a ton of unneeded hyphens between each word. This will hopefully fix that going forward.
def unneeded_space_remover(link_content_str):
    content_str_split = link_content_str.split()
    final_content_str = str("")

    for word in content_str_split:
        if word != " ":
            final_content_str = final_content_str + word + " "
        else:
            pass
